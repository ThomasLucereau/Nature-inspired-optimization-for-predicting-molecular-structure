...
def simulated_annealing(fonction, pt_depart, temp_initial, n_etapes):
    pt_en_cours = pt_depart
    chemin = [pt_depart]
    k = temp_initial / n_etapes  # Calcul du coefficient k pour le modèle linéaire
    ...
    for step in range(n_etapes):
        ...
        if delta < 0 or (delta > 0 and np.exp(-delta / temp_initial) > np.random.rand()):
            pt_en_cours = voisin
            chemin.append(pt_en_cours)
        temp_initial -= k  # Modèle linéaire pour la décroissance de la température
...
chemin = simulated_annealing(f, pt_depart, temp_initial, n_etapes)
