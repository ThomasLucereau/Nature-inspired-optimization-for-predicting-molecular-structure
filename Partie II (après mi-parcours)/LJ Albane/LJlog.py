...
# Fonction de décroissance logarithmique pour la température
...
def simulated_annealing(fonction, pt_depart, temp_initial, n_etapes, nb_atoms):
    pt_en_cours = pt_depart
    chemin = [pt_depart]
    E_hist = [fonction(pt_depart, nb_atoms)]

    for step in range(1, n_etapes + 1):
        voisin = pt_en_cours + np.random.uniform(-0.05, 0.05, pt_en_cours.shape)  # Augmentation de l'amplitude des perturbations
        voisin = np.clip(voisin, -2, 2)
        ...
        if delta < 0 or (delta > 0 and np.random.rand() < np.exp(-delta / temp_actuelle)):
            ...
            E_hist.append(fonction(pt_en_cours, nb_atoms))
...
def run_simulated_annealing(nb_atoms, n_trials=100):
    ...
    for _ in range(n_trials):
        ...
        temp_initial = 1000  # Température initiale plus basse
        n_etapes = 100000  # Augmentation du nombre d'étapes

        # Exécute le recuit simulé
        resultat = simulated_annealing(f, pt_depart, temp_initial, n_etapes, nb_atoms)
