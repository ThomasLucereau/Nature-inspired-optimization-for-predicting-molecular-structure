...
def simulated_annealing(fonction, pt_depart, temp_initial, n_etapes, nb_atoms):
    ...
    k = temp_initial / n_etapes
    
    for step in range(1, n_etapes + 1):
        temp_actuelle = logarithmic_decay(temp_initial, step)
        perturbation = np.random.normal(0, 0.05 * temp_actuelle / temp_initial, pt_en_cours.shape)
        voisin = pt_en_cours + perturbation
...
def run_simulated_annealing(nb_atoms, n_trials=100):
    ...
    for _ in range(n_trials):
        ...
        temp_initial = 1000  # Température initiale
        n_etapes = 100000  # Nombre d'étapes
